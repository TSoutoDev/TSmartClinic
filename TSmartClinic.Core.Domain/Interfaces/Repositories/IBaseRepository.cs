...
namespace TSmartClinic.Core.Domain.Interfaces.Repositories
{
    public interface IBaseRepository<TEntity> : IDisposable where TEntity : Base
    {
        ...
        TEntity ObterPorId(int id, params Expression<Func<TEntity, object>>[] properties);// uso interno
        TEntity ObterPorPublicId(Guid publicId, params Expression<Func<TEntity, object>>[] properties);// uso externo
